...
def bin_data(t,f,err=None,statistic="mean",bins=20):
	...
	err : array-like
		Flux uncertainties.
	statistic : str
		Statistic to compute in each bin. ['mean','median','std','count','sum','max'] See `scipy.stats.binned_statistic`.
	...
	try:
		...
		t_bin = 0.5*(y_binedges[1:] + y_binedges[:-1])  # center of the bins
		nans = np.isnan(y_bin)
...
def bin_data_with_gaps(t,f,e=None, statistic="mean", binsize=0.0104, gap_threshold=0.5, exact_binsize=True, verbose=False):
	"""
	...
	statistic : str, optional
		Statistic to compute in each bin. ['mean','median','std','count','sum','max'] See `scipy.stats.binned_statistic`. Default is 'mean'.
	binsize : float, optional
		Size of the bins. default is 0.0104 (~15 mins)
	gap_threshold : float, optional
		Threshold for identifying gaps. default is 0.5*binsize
	exact_binsize : bool, optional
		If True, use exact binsize for binning. If False, use equally spaced bins based on the 
	...
	"""
	if binsize==0 or binsize<=2*np.median(np.diff(t)):  # no binning, if binsize is zero or less than twice the data cadence
		return (t,f) if e is None else (t,f,e)
	try:
		gap 	= np.diff(t)
		gap 	= np.insert(gap,0,0)  # add a zero at the start to align the lengths
		gap_ind = np.where(gap > gap_threshold*binsize)[0]  # gap index is the start of the next chunk

		#split t into chunks by the gaps
		t_chunks = np.split(t, gap_ind) 
		f_chunks = np.split(f, gap_ind)
		e_chunks = np.split(e, gap_ind) if e is not None else f_chunks

		t_binned, f_binned, e_binned = [],[],[]
		if verbose:
			print(f"Before binning, divided data into {len(t_chunks)} chunks based on gaps larger than {gap_threshold*binsize}")
		
		for tc,fc,ec in zip(t_chunks,f_chunks,e_chunks):
			if np.ptp(tc) < binsize: 
				if exact_binsize:
					bins = np.array([np.min(tc), np.max(tc)])
				else:
					bins = 1
			else: 
				if exact_binsize:
					bins = np.arange(np.min(tc), np.max(tc)+0.25*binsize, binsize)
				else:
					bins = int(np.ptp(tc)/binsize)
			if e is not None: 
				t_bin, f_bin, e_bin = bin_data(tc,fc,ec,statistic=statistic,bins=bins)
			else: 
				t_bin, f_bin = bin_data(tc,fc,statistic=statistic,bins=bins)
		...
		return bin_data(t,f,e,statistic=statistic,bins=int(np.ptp(t)/binsize))
...
def cosine_atm_variation(phi, Fd=0, Fn=0, delta_deg=0,cosine_order=1):
	...
	"""
	res        = SimpleNamespace()
	res.delta  = np.deg2rad(delta_deg)      # hotspot offset in radians
	res.delta_phase = res.delta/(2*np.pi)   # in phase units
...
def CA_08_PCmodel(phi, Fd, C1, D1, C2, D2):
	"""
	...
	--------
	F : array-like
		planetary flux as a function of phase angle
	...
	"""
	res         = SimpleNamespace()
	theta       = phi - np.pi # phase angle shifted by pi so that theta=0 at secondary eclipse
	res.pc      = Fd + C1*(np.cos(theta) - 1) + D1*np.sin(theta) + C2*(np.cos(2*theta) - 1) + D2*np.sin(2*theta)
	res.Fmin    = min(res.pc)
	res.Fmax    = max(res.pc)
...
def lambertian_atm_variation(phi, Fd=0, Fn=0, delta_deg=0,inc_rad=np.pi/2, night_phase_var=False):
	...
	alpha_day  = np.arccos( -np.sin(inc_rad)*np.cos(phi+res.delta) ) #see eq. 3 of Esteves+2013 (https://iopscience.iop.org/article/10.1088/0004-637X/772/1/51)
